class K8::Helm::Redis < K8::Helm::Service
  ...
  def password
    output = K8::Kubectl.new(connection, Cli::RunAndReturnOutput.new).call("get secret --namespace #{add_on.name} #{service_basename} -o jsonpath='{.data.redis-password}' | base64 -d")
    output.strip
  end
end
